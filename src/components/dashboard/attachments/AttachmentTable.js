      ...
      <div className="table-responsive-material">
        <table className="default-table table-unbordered table table-sm table-hover">
          ...
          <tbody>
          {
            ...
            ? props.data.map(data => {
              return (
                <AttachmentTableCell openModal={(data) => {props.openModal(data)}} deleteAttachment={(id) => {deleteAttachment(id)}} getAttachments={props.getAttachments} key={data._id} data={data}/>
              );
            })
            : null
          }
