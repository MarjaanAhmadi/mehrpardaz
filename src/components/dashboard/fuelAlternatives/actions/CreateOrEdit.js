...
const CreateOrEdit = (props) => {
  ...
  const craeteFuelAlternative = async () => {
    try {
      ...
      data[
        "alternative_f_time"
      ] = `${form.alternative_f_time._d.getHours()}:${form.alternative_f_time._d.getMinutes()}`;
  ...
  const editFuelAlternative = async () => {
    try {
      const data = form;
      data["alternative_f_time"] = `${
        form.alternative_f_time._d.getHours() < 10
          ? `0${form.alternative_f_time._d.getHours()}`
          : form.alternative_f_time._d.getHours()
      }:${
        form.alternative_f_time._d.getMinutes() < 10
          ? `0${form.alternative_f_time._d.getMinutes()}`
          : form.alternative_f_time._d.getMinutes()
      }`;
       ;
      await Patch("fuel-alternatives", data, props.id);
      setSuccess({
        ...success,
        hasSuccess: true,
  ...
  return (
    <React.Fragment>
      ...
      <div className={`jr-btn-group d-flex flex-wrap mt-3 ${classes.btnRoot}`}>
        <Button
          onClick={!props.edit ? craeteFuelAlternative : editFuelAlternative}
          variant="contained"
          color="primary"
