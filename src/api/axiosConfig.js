  ...
  async (error) => {
    ...
    if (error.config && error.response && error.response.status === 401) {
      ...
      localStorage.setItem(
        "refresh-token",
        response.data.message.refresh.token
      );

      localStorage.setItem(
        "token",
        `Bearer ${response.data.message.access.token}`
      );
      return sendReq(
        error.config,
        response.data.message.access.token
      ).then((response) => {});
    }
...
axiosInstance.interceptors.request.use(
  (reqConfig) => {
    if (!reqConfig.url.includes("/token-auth")) {
      ...
      reqConfig.headers["Organization"] = localStorage.getItem(
        "organizationId"
      );
      return reqConfig;
    }
